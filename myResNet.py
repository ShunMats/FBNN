...
class myResNet:
    ...
    def res_blocks(x,hidden_node): 
        conv = Dense(hidden_node)(x)
        conv = BatchNormalization()(conv)
        conv = Activation("relu")(conv)
        conv = Dense(hidden_node)(conv)
        conv = BatchNormalization()(conv)
    ...
    def make_model(self):
        ...
        for _ in range(int((self.hidden_layer -2)/2)):
            x = res_blocks(x,self.hidden_node)
    ...
    def res_blocks_leakyReLU(x,hidden_node,leaky_alpha): 
        conv = Dense(hidden_node)(x)
        conv = BatchNormalization()(conv)
        conv = LeakyReLU(alpha=leaky_alpha)(conv)

        conv = Dense(hidden_node)(conv)
        conv = BatchNormalization()(conv)
        short_cut = shortcut(conv, x)
        conv = LeakyReLU(alpha=leaky_alpha)(short_cut)
        return conv
    ...
    def make_model_leakyReLU(self):
        ...
        for _ in range(int((self.hidden_layer -2)/2)):
            x = res_blocks_leakyReLU(x,self.hidden_node,self.leaky_alpha)
